...
class EmbeddingStore:
    ...
    def __init__(
        ...
        # Инициализируем клиент OpenAI для создания эмбеддингов через ProxyAPI
        # API ключ берется из параметра или переменной окружения PROXY_API_KEY
        # Используем ProxyAPI вместо прямого OpenAI API
        ...
        print(f"Модель эмбеддингов: {embedding_model} (ProxyAPI/OpenAI)")
